        ...
        for b in bots_list:
            ...
            if self.mode == WEBHOOK_MODE:
                try:
                    ...
                    dispatcher = Dispatcher(bot, None, workers=workers)
                    ...
                    setted = bot.setWebhook(hookurl, certificate=certificate, timeout=timeout, max_connections=max_connections,
                ...
                try:
                    updater = Updater(token=token, request_kwargs=proxy, workers=workers, use_context=context)
                    bot = updater.bot
                    bot.delete_webhook()
